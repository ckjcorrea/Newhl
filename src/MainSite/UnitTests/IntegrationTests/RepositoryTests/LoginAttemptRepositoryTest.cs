...
namespace Newhl.MainSite.DevDefined.UnitTests.IntegrationTests.RepositoryTests
{
    ...
    public class LoginAttemptRepositoryTest : RepositoryTestBase
    {
        ...
        {
            LoginAttempt testItem = this.GenerateLoginAttempt(Newhl.MainSite.UnitTests.Constants.UserConstants.TestUserName, true);
        ...
        public void LoginAttemptRepositoryTestGetById()
        {
            ...
            if(testItem == null)
            {
                testItem = this.GenerateLoginAttempt(Newhl.MainSite.UnitTests.Constants.UserConstants.TestUserName, true);
                testItem = this.RepositoryManager.LoginAttemptRepository.Save(testItem);
        ...
        {
            IList<LoginAttempt> testItems = this.RepositoryManager.LoginAttemptRepository.GetByUserName(Newhl.MainSite.UnitTests.Constants.UserConstants.TestUserName);

            if (testItems == null || testItems.Count == 0)
            {
                LoginAttempt testItem = this.GenerateLoginAttempt(Newhl.MainSite.UnitTests.Constants.UserConstants.TestUserName,true);
                testItem = this.RepositoryManager.LoginAttemptRepository.Save(testItem);
                testItems = this.RepositoryManager.LoginAttemptRepository.GetByUserName(Newhl.MainSite.UnitTests.Constants.UserConstants.TestUserName);
            }
